...
class PassingLane:
    ...
    distance: float
    # (player_id, distance to passing lane)
    interceptors: List[Tuple[int, float]]
...
class PassingOpportunitiesAnalyzer:
    ...
    def calculate_pass_success_probability(self, passing_lane: PassingLane,
        ...
        interceptor_risk = sum(1.0 / (1.0 + 2*dist/distance)
                               for _, dist in passing_lane.interceptors)
    ...
    def calculate_tactical_advantage(self, receiver_pos: Tuple[float, float],
        ...
        current_control = self.calculate_space_control(
            team_positions, opponent_positions)
        new_positions = team_positions[:]
        new_positions.append(receiver_pos)
        new_control = self.calculate_space_control(
            new_positions, opponent_positions)
    ...
    def analyze_passing_opportunities(self,
        ...
        for receiver_id, receiver_pos in teammate_positions:
            ...
            horizontal_progress = (
                receiver_pos[0] - passer_pos[0]) / self.field_length
    ...
    def calculate_reward_score(self, horizontal_progress: float,
        ...
        horizontal_progress_norm = (horizontal_progress + 1) / 2
        # Assume 20m is max space gain
        space_gained_norm = min(1.0, space_gained / 20)
