# soccer_analytics/core/space_control_analyzer.py
...
class SpaceControlAnalyzer:
    def __init__(self, config_path: str = f'{os.path.dirname(os.path.realpath(__file__))}/../config/config.yaml'):
        ...
        self.grid_points = np.array([(x, y) for x in self.x_grid for y in self.y_grid])
    ...
    def calculate_arrival_time(self,
        ...
        distance_to_max = current_speed * time_to_max + 0.5 * self.acceleration * time_to_max**2
    ...
    def calculate_control_score(self,
        ...
        arrival_time = self.calculate_arrival_time(player_pos, player_vel, point)
    ...
    def analyze_space_control(self,
        ...
        """
        all_positions = [(pid, pos, vel, 'home') for pid, pos, vel in home_positions]
        all_positions.extend([(pid, pos, vel, 'away') for pid, pos, vel in away_positions])
        ...
        for i, point in enumerate(self.grid_points):
            for j, (_, pos, vel, _) in enumerate(all_positions):
                control_matrix[i, j] = self.calculate_control_score(point, pos, vel)
        ...
        self.dominant_regions = dominant_team.reshape(len(self.x_grid), len(self.y_grid))
        self.control_surface = dominance_strength.reshape(len(self.x_grid), len(self.y_grid))
        
        # Create control zones for each player
        self.control_zones = []
        for pid, pos, vel, team in all_positions:
            # Calculate individual control area
            player_control = control_matrix[:, all_positions.index((pid, pos, vel, team))]
            controlled_points = self.grid_points[player_control > 0.5 * np.max(player_control)]
            
            zone = ControlZone(
                position=pos,
                velocity=vel,
                controlled_area=len(controlled_points) * self.grid_resolution**2,
    ...
    def _calculate_zone_control(self, start_y: float, end_y: float) -> Dict:
        ...
        zone_mask = (self.grid_points[:, 1] >= start_y) & (self.grid_points[:, 1] < end_y)
    ...
    def visualize_space_control(self):
        ...
        for zone in self.control_zones:
            ...
            plt.plot(zone.position[0], zone.position[1], 'o', color=color, markersize=8)
    ...
    def plot_control_evolution(self, control_history: List[Dict]):
        ...
        home_control = [frame['space_control']['home'] for frame in control_history]
        away_control = [frame['space_control']['away'] for frame in control_history]
