...
keypoint_world_coords_2D = [[0., 0.], [52.5, 0.], [105., 0.], [0., 13.84], [16.5, 13.84], [88.5, 13.84], [105., 13.84],
                            [0., 24.84], [5.5, 24.84], [99.5, 24.84], [
                                105., 24.84], [0., 30.34], [0., 30.34],
                            [105., 30.34], [105., 30.34], [0., 37.66], [
                                0., 37.66], [105., 37.66], [105., 37.66],
                            [0., 43.16], [5.5, 43.16], [99.5, 43.16], [
                                105., 43.16], [0., 54.16], [16.5, 54.16],
                            [88.5, 54.16], [105., 54.16], [0., 68.], [
                                52.5, 68.], [105., 68.], [16.5, 26.68],
                            [52.5, 24.85], [88.5, 26.68], [16.5, 41.31], [
                                52.5, 43.15], [88.5, 41.31], [19.99, 32.29],
                            [43.68, 31.53], [61.31, 31.53], [85., 32.29], [
                                19.99, 35.7], [43.68, 36.46], [61.31, 36.46],
                            [85., 35.7], [11., 34.], [16.5, 34.], [
                                20.15, 34.], [46.03, 27.53], [58.97, 27.53],
                            [43.35, 34.], [52.5, 34.], [61.5, 34.], [
                                46.03, 40.47], [58.97, 40.47], [84.85, 34.],
                            [88.5, 34.], [94., 34.]]  # 57

keypoint_aux_world_coords_2D = [[5.5, 0], [16.5, 0], [88.5, 0], [99.5, 0], [5.5, 13.84], [99.5, 13.84], [16.5, 24.84],
                                [88.5, 24.84], [16.5, 43.16], [88.5, 43.16], [
                                    5.5, 54.16], [99.5, 54.16], [5.5, 68],
...
line_world_coords_3D = [[[0., 54.16, 0.], [16.5, 54.16, 0.]], [[16.5, 13.84, 0.], [16.5, 54.16, 0.]],
                        ...
                        [[52.5, 0., 0.], [52.5, 68, 0.]], [[0., 68., 0.], [
                            105., 68., 0.]], [[0., 0., 0.], [0., 68., 0.]],
                        [[105., 0., 0.], [105., 68., 0.]], [[0., 0., 0.], [
                            105., 0., 0.]], [[0., 43.16, 0.], [5.5, 43.16, 0.]],
...
keypoint_world_coords_2D = [[x - 52.5, y - 34]
                            for x, y in keypoint_world_coords_2D]
keypoint_aux_world_coords_2D = [[x - 52.5, y - 34]
                                for x, y in keypoint_aux_world_coords_2D]
line_world_coords_3D = [[[x1 - 52.5, y1 - 34, z1], [x2 - 52.5, y2 - 34, z2]]
...
class FramebyFrameCalib:
    ...
    def get_keypoints_subsets(self):
        ...
        for kp in self.keypoints_dict.keys():
            wp = keypoint_world_coords_2D[kp -
                                          1] if kp <= 57 else keypoint_aux_world_coords_2D[kp - 1 - 57]
    ...
    def get_per_plane_correspondences(self, mode, use_ransac):
        ...
        for kp in keypoints.keys():
            if kp in [12, 16]:
                keys_p2.append(kp)
                world_points_p2.append(
                    [-keypoints[kp]['zw'], keypoints[kp]['yw'], 0.])
                img_points_p2.append(
                    [keypoints[kp]['xi'], keypoints[kp]['yi']])
                ...
                world_points_p1.append(
                    [keypoints[kp]['xw'], keypoints[kp]['yw'], keypoints[kp]['zw']])
                world_points_p3.append(
                    [-keypoints[kp]['zw'], keypoints[kp]['yw'], 0.])
                img_points_p1.append(
                    [keypoints[kp]['xi'], keypoints[kp]['yi']])
                img_points_p3.append(
                    [keypoints[kp]['xi'], keypoints[kp]['yi']])
            elif kp in [15, 19]:
                keys_p3.append(kp)
                world_points_p3.append(
                ...
                if len(obj_list[i]) >= 4 and not all(item[0] == obj_list[i][0][0] for item in obj_list[i]) \
                    ...
                    if i == 0:
                        h, status = cv2.findHomography(
                            np.array(obj_list[i]), np.array(img_list[i]), cv2.RANSAC, use_ransac)
                        obj_list[i] = [obj for count, obj in enumerate(
                            obj_list[i]) if status[count] == 1]
                        img_list[i] = [obj for count, obj in enumerate(
                            img_list[i]) if status[count] == 1]
                        key_list[i] = [obj for count, obj in enumerate(
                            key_list[i]) if status[count] == 1]
    ...
    def get_correspondences(self, mode):
        ...
        for kp in keypoints:
            obj_pts.append([self.subsets[mode][kp]['xw'], self.subsets[mode]
                           [kp]['yw'], self.subsets[mode][kp]['zw']])
            img_pts.append([self.subsets[mode][kp]['xi'],
                           self.subsets[mode][kp]['yi']])
    ...
    def change_plane_coords(self, w=105, h=68):
        ...
        if self.ord_pts[0] == 1:
            self.position = np.linalg.inv(
                R) @ self.position + np.array([-w/2, 0, 0])
        ...
        if ret:
            ...
            cam_params = {"pan_degrees": pan,
                          ...
                          "rotation_matrix": [[self.rotation[0, 0], self.rotation[0, 1], self.rotation[0, 2]],
                                              [self.rotation[1, 0], self.rotation[1,
                                                                                  1], self.rotation[1, 2]],
    ...
    def get_homography_from_ground_plane(self, use_ransac=5., inverse=False):
        self.get_per_plane_correspondences(
            mode='ground_plane', use_ransac=use_ransac)
        obj_pts, img_pts = self.get_correspondences('ground_plane')

        if len(obj_pts) >= 4:
            H, mask = cv2.findHomography(
                obj_pts, img_pts, cv2.RANSAC, use_ransac)
    ...
    def get_homography_from_3D_projection(self, use_ransac=5., inverse=False):
        cam_params, ret = self.get_cam_params(
            mode='full', use_ransac=use_ransac)
    ...
    def heuristic_voting(self):
        ...
        for mode in ['full', 'ground_plane', 'main']:
            for use_ransac in [0, 5, 10, 15, 25, 50]:
                cam_params, ret = self.get_cam_params(
                    mode=mode, use_ransac=use_ransac)
                if ret:
