...
def resize_keypoints(keypoints, original_size, new_size):
    ...
    for kp, values in keypoints.items():
        ...
        resized_keypoints[kp] = {'x': x_resized,
                                 'y': y_resized, 'in_frame': values['in_frame']}
...
def generate_gaussian_array_vectorized(num_matrices, keypoints, original_size, down_ratio=2, sigma=2, proj_err_th=5.):
                ...
                if 'proj_err' in resized_keypoints[kp].keys():
                    if resized_keypoints[kp]['proj_err'] <= proj_err_th:
                        center_points.append(
                            [resized_keypoints[kp]['x'], resized_keypoints[kp]['y']])
                    else:
    ...
    matrices = [generate_gaussian_matrix_vectorized(
        new_size[0], new_size[1], px, py, sigma) for px, py in center_points]
    matrices = np.array(matrices)
    matrices = np.concatenate(
        (matrices, 1-matrices.sum(axis=0, keepdims=True)), axis=0)
...
def resize_keypoints_l(keypoints, original_size, new_size):
    ...
    for kp, values in keypoints.items():
        ...
        resized_keypoints[kp] = {
            'x_1': x1_resized, 'y_1': y1_resized, 'x_2': x2_resized, 'y_2': y2_resized}
...
def generate_gaussian_array_vectorized_l(num_matrices, keypoints, original_size, down_ratio=2, sigma=2, sigma_mult=1):
    ...
    for kp in range(1, num_matrices+1):
        if kp in resized_keypoints.keys():
            center_points.append(
                [resized_keypoints[kp]['x_1'], resized_keypoints[kp]['y_1']])
            center_points.append(
                [resized_keypoints[kp]['x_2'], resized_keypoints[kp]['y_2']])
        ...
        if kp in resized_keypoints.keys():
            ...
            if num_gaussians != 1:
                for i in range(num_gaussians):
                    ...
                    matrices_border[0, :, :] += generate_gaussian_matrix_vectorized(
                        new_size[0], new_size[1], x, y, sigma)
            else:
...
def get_keypoints_from_heatmap_batch_maxpool(
    ...
    scores, indices = torch.topk(heatmap.view(
        batch_size, n_channels, -1), max_keypoints, sorted=True)
    indices = torch.stack(
    ...
    filtered_indices = [[[]
                         for _ in range(n_channels)] for _ in range(batch_size)]
...
def get_keypoints_from_heatmap_batch_maxpool_l(
    ...
    max_pooled_heatmap = torch.nn.functional.max_pool2d(
        heatmap, kernel, stride=1, padding=pad)
    # if the value equals the original value, it is the local maximum
    local_maxima = max_pooled_heatmap == heatmap
    ...
    for batch in range(len(kp_dict)):
        ...
        for key in range(1, 31):
            if key not in kp_dict[batch].keys():
                ...
                line_key1, line_key2 = lines_list.index(
                    line_keys[0]) + 1, lines_list.index(line_keys[1]) + 1
                if all(line_key in lines_dict[batch].keys() for line_key in [line_key1, line_key2]):
                    x1 = [lines_dict[batch][line_key1]['x_1'],
                          lines_dict[batch][line_key1]['x_2']]
                    y1 = [lines_dict[batch][line_key1]['y_1'],
                          lines_dict[batch][line_key1]['y_2']]
                    x2 = [lines_dict[batch][line_key2]['x_1'],
                          lines_dict[batch][line_key2]['x_2']]
                    y2 = [lines_dict[batch][line_key2]['y_1'],
                          lines_dict[batch][line_key2]['y_2']]
                    new_kp = line_intersection(x1, y1, x2, y2)
                    if -w_extra < new_kp[0] < w_extra + w and -h_extra < new_kp[1] < h_extra + h:
                        complete_dict[key] = {'x': round(
                            new_kp[0], 0), 'y': round(new_kp[1], 0), 'p': 1.}
